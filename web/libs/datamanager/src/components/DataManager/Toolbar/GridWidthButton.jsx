...
export const GridWidthButton = injector(({ view, isGrid, gridWidth, fitImagesToWidth, hasImage, size }) => {
  ...
  return isGrid ? (
      ...
      content={
        <div className="p-tight min-w-wide space-y-base">
          <div className="grid grid-cols-[1fr_min-content] gap-base items-center">
            <span>{i18next.t('datamanager.components.DataManager.Toolbar.GridWidthButton.columnsWidth', { ns: "datamanager", defaultValue: "Columns: {{width}}", width })}</span>
            <ButtonGroup collapsed={false}>
              <Button
                ...
                leading={<IconMinus />}
                size="small"
                aria-label={i18next.t('datamanager.components.DataManager.Toolbar.GridWidthButton.decreaseColumnsNumber', { ns: "datamanager", defaultValue: "Decrease columns number" })}
              />
              <Button
                onClick={() => setGridWidth(width + 1)}
                disabled={width === 10}
                ...
                leading={<IconPlus />}
                size="small"
                aria-label={i18next.t('datamanager.components.DataManager.Toolbar.GridWidthButton.increaseColumnsNumber', { ns: "datamanager", defaultValue: "Increase columns number" })}
              />
            </ButtonGroup>
          </div>
          {hasImage && (
            <div className="grid grid-cols-[1fr_min-content] gap-base items-center">
              <span>{i18next.t('datamanager.components.DataManager.Toolbar.GridWidthButton.fitImagesToWidth', { ns: "datamanager", defaultValue: "Fit images to width" })}</span>
    ...
    >
      <Button size={size} variant="neutral" look="outlined" aria-label={i18next.t('datamanager.components.DataManager.Toolbar.GridWidthButton.gridSettings', { ns: "datamanager", defaultValue: "Grid settings" })}>
        <IconSettings />
      </Button>
    </Dropdown.Trigger>
  ) : null;
